...
RUN curl -fsSL "https://go.dev/dl/go${GO_VERSION}.linux-amd64.tar.gz" -o /tmp/go.tar.gz \
    ...
    && rm /tmp/go.tar.gz
ENV PATH="/usr/local/go/bin:/home/${RDE_USER}/go/bin:${PATH}"
...
# Use --user-data-dir to ensure extensions install into the workspace user's data directory
RUN code-server --user-data-dir /home/${RDE_USER}/.local/share/code-server \
      --install-extension anthropic.claude-code 2>/dev/null || true \
    && code-server --user-data-dir /home/${RDE_USER}/.local/share/code-server \
...
# Copied to /home/${RDE_USER}/project/ at startup by entrypoint.sh (only if not already present)
...
RUN mkdir -p /home/${RDE_USER}/.local/share/code-server/extensions/qovery.builder-startup-0.0.1 \
    && cp /tmp/builder-startup-extension/package.json \
          /tmp/builder-startup-extension/extension.js \
          /home/${RDE_USER}/.local/share/code-server/extensions/qovery.builder-startup-0.0.1/ \
    && rm -rf /tmp/builder-startup-extension \
    && jq --arg home "/home/${RDE_USER}" '. += [{"identifier":{"id":"qovery.builder-startup"},"version":"0.0.1","location":{"$mid":1,"path":($home + "/.local/share/code-server/extensions/qovery.builder-startup-0.0.1"),"scheme":"file"},"relativeLocation":"qovery.builder-startup-0.0.1"}]' \
       /home/${RDE_USER}/.local/share/code-server/extensions/extensions.json > /tmp/ext.json \
    && mv /tmp/ext.json /home/${RDE_USER}/.local/share/code-server/extensions/extensions.json
...
RUN mkdir -p /home/${RDE_USER}/.config/code-server \
    && printf 'bind-addr: 0.0.0.0:8080\nauth: none\ncert: false\napp-name: Builder Workspace\n' \
       > /home/${RDE_USER}/.config/code-server/config.yaml
...
RUN mkdir -p /home/${RDE_USER}/.local/share/code-server/User \
    && printf '%s\n' \
       '{' \
...
RUN mkdir -p /home/${RDE_USER}/.config/opencode/skills \
    /home/${RDE_USER}/.config/opencode/commands \
    /home/${RDE_USER}/.claude/skills

# Qovery Skills — installed as workspace user so OpenCode and Claude Code can discover them
...
# Start as root so the entrypoint can fix /home/${RDE_USER} ownership when a volume
# is mounted at /home (the entrypoint drops to the workspace user after fixing).
USER root
COPY entrypoint.sh /usr/local/bin/entrypoint.sh
RUN chmod +x /usr/local/bin/entrypoint.sh
RUN mkdir -p /etc/rde && printf 'RDE_USER="%s"\nRDE_HOME="/home/%s"\n' "${RDE_USER}" "${RDE_USER}" > /etc/rde/env
